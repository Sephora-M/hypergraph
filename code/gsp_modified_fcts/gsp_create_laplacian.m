...
if isfield(G,'hypergraph') && G.hypergraph
    G.de = sum(G.W >0,1)';
    G.dv = sum(G.W.^2,2);
    G.dve = sum(G.W*G.de,2);
     switch type
         case 'normalized'   % I - D^{-1/2}*H*W*De^*D^{-1/2}     
         ...
         case 'unnormalizedA' % Dv - H*W*De^1*H^t
            G.A = G.W*G.W' - diag(G.dv);
            G.L = diag(G.dv) -  G.W * ...
                diag(G.de.^(-1)) * ...
                G.W' ;
            G.lap_type = 'non-normalizedA';
         case 'unnormalizedB' % Dve - HWH^t
            G.A = G.W*G.W' - diag(G.dv);
            
            G.L = diag(G.dve) -  G.W * ...
                G.W' ;
            G.lap_type = 'non-normalizedB';
         case 'unnormalizedC' % L - A
            G.A = G.W*G.W' - diag(G.dv);
            G.L = diag(G.dv) - G.A;
         otherwise
